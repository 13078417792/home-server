...
class Base extends \think\Controller {
	...
	const BACK_PATH = APP_PATH.DS.'back';
	...
	protected function _initialize() {
		...
		$this->auth = $auth ?: '';
		$auth_detail = $auth ? Cache::tag('auth')->get('auth_'.$auth) : [];
		...
		if (!Rbac::isWhite($path) && empty($auth)) {
			self::printJson(false, '请登录');
		}

//        self::printJson(false, '',['path'=>$path,'isWhite'=>Rbac::isWhite($path)]);
		...
		$GLOBALS['USER_DETAIL'] = $auth_detail;
	...
	static public function hasUserDetail($send = false) {
		if (!(
		...
		if (!$result && $send) {
			self::printJson('false', '没有用户数据', ['error' => '没有用户数据']);
		} else {
			return $result;
		}
	}
	...
	static public function unserialize_auth_token($token) {
		...
		$cache = json_decode(Cache::tag('auth')->get('auth_'.$token, '{}'), true);
		if (empty($cache)) {
			return [];
		}
